...
You can also scrape links with a variety of different Google News keywords, and save them in one result. The method is to use the same `result_set` when running the second `scrape_google_news_link()`.
...
```python
    ...
    news_dict, status_list = scrape_news_text('results.csv', lang='id')
